<template>
  <div>
    ...
    <el-dialog title="入库记录" :model-value="dialogVisible" width="520px" @close="closeDialog">
...
const loadList = async () => {
  try {
    ...
    const records = await inboundApi.list()
...
const submitForm = async () => {
  try {
    ...
    if (isEdit.value) {
      await inboundApi.update(currentId.value, payload)
...
const confirmDelete = row => {
  ElMessageBox.confirm('确认删除该入库记录吗？', '请确认', { type: 'warning' })
    .then(async () => {
      try {
